    ...
    if bed_share == "Volledige beddeldeling":
        ...
        for i in range(0, n_loc):
            with st.expander(st.text_input("",value="Location "+str(i+1))):
                ...
                beds_ELV_High = st.number_input(f"Number of ELV High Complex beds", min_value=0, step=1, value=0, format="%d")
                nurs_ELV_High = st.number_input(f"Number of ELV High Complex nurses", min_value=0, step=1, value=0, format="%d")
